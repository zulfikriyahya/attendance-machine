...
static void saveEncryptedNvs(const char *ns, const char *key, const char *plain) {
  ...
  if (!encryptString(plain, ec)) return;
...
static bool loadEncryptedNvs(const char *ns, const char *key, char *plain, size_t maxLen) {
  ...
  prefs.end();
  return decryptString(ec, plain, maxLen);
}
...
static WiFiClientSecure &getHttpClient() {
  ...
  _httpClient.setAlpnProtocols(alpn);
  return _httpClient;
}
...
void restoreWdtNormal() {
  ...
  // esp_task_wdt_delete(nullptr);
  // esp_task_wdt_deinit();
  // const esp_task_wdt_config_t cfg = { .timeout_ms = WDT_TIMEOUT_SEC * 1000u, .idle_core_mask = 0, .trigger_panic = true };
  // esp_task_wdt_init(&cfg);
  // esp_task_wdt_add(nullptr);
  // wdtExtended = false;
}
...
bool loadRfidCacheFromFileLocked() {
  clearRfidCache();
  if (!sd.exists(RFID_DB_FILE)) return false;
  FsFile f;
  if (!f.open(RFID_DB_FILE, O_RDONLY)) return false;
  char line[12];
  int idx = 0;
...
void checkOLEDSchedule() {
  ...
  int h = ti.tm_hour;
  if (h >= rtCfg.dimStartHour && h < rtCfg.dimEndHour) turnOffOLED();
  else turnOnOLED();
}

/* ===================== FAILED LOG (SIZE-LIMITED) ===================== */
void appendFailedLog(const char *rfid, const char *ts, const char *reason) {
  ...
  if (sd.exists("/failed_log.csv")) {
    if (file.open("/failed_log.csv", O_RDONLY)) {
      char ln[128];
      while (file.fgets(ln, sizeof(ln)) > 0) lineCount++;
      file.close();
    }
  }
  ...
  if (file.open("/failed_log.csv", O_WRONLY | O_CREAT | O_APPEND)) {
    if (file.size() == 0) file.println(F("rfid,timestamp,reason"));
    file.print(rfid);
    file.print(',');
    file.print(ts);
    file.print(',');
...
int countRecordsInFileLocked(const char *filename) {
  ...
  while (file.fgets(line, sizeof(line)) > 0) {
    ...
    if (len > 10) {
      ...
      char *c2 = c3 ? (char *)memrchr(line, ',', c3 - line) : nullptr;
      char *c1 = c2 ? strchr(line, ',') : nullptr;
      if (!c1 || !c2 || !c3) {
        cnt++;
        continue;
      }
      unsigned long t = strtoul(c3 + 1, nullptr, 10);
      const char *rfidEnd = c1;
      size_t rlen = rfidEnd - line;
      if (rlen <= 10) {
        char rfidBuf[11];
        memcpy(rfidBuf, line, rlen);
        rfidBuf[rlen] = '\0';
        uint8_t expected = recordCrc8(rfidBuf, t);
        char crcStr[4];
        snprintf(crcStr, sizeof(crcStr), "%02X", expected);
        (void)crcStr;
      }
      cnt++;
    }
...
bool isDuplicateLocked(const char *rfid, unsigned long t) {
  ...
  for (int offset = 0; offset < MAX_DUPLICATE_CHECK_FILES; offset++) {
    ...
    char line[128];
    if (file.available()) file.fgets(line, sizeof(line));  // skip header
    int read = 0;
    bool found = false;
    while (!found && read < MAX_DUPLICATE_CHECK_LINES && file.fgets(line, sizeof(line)) > 0) {
      ...
      // Format: rfid,timestamp,device_id,unix_time,crc8
      char *c1 = strchr(line, ',');  // setelah rfid
      if (!c1) {
        read++;
        continue;
      }
      char *c2 = strchr(c1 + 1, ',');  // setelah timestamp
      if (!c2) {
        read++;
        continue;
      }
      char *c3 = strchr(c2 + 1, ',');  // setelah device_id
      if (!c3) {
        read++;
        continue;
      }
      char *c4 = strchr(c3 + 1, ',');  // setelah unix_time
      // c4 boleh null kalau tidak ada kolom crc8

      // Ambil rfid
      *c1 = '\0';
      // Ambil unix_time dari c3+1 sampai c4 (atau akhir string)
      unsigned long ft;
      if (c4) {
        *c4 = '\0';
      ...
      if (strcmp(line, rfid) == 0 && ft > 0 && t >= ft && (t - ft) < MIN_REPEAT_INTERVAL)
      Serial.printf("dupCheck: DUPLIKAT DITEMUKAN!\n");
        found = true;
...
SaveResult saveToQueue(const char *rfid, const char *ts, unsigned long t) {
  if (!sdCardAvailable) return SAVE_SD_ERROR;
  if (!acquireSD()) return SAVE_SD_ERROR;
  selectSD();
...
void checkAndUpdateRfidDb() {
  ...
  if (server == 0 || server <= local) return;
  downloadRfidDb();
}
...
void fetchRemoteConfig() {
  ...
  if (doc.containsKey("sleep_start")) rtCfg.sleepStartHour = doc["sleep_start"];
  if (doc.containsKey("sleep_end")) rtCfg.sleepEndHour = doc["sleep_end"];
  if (doc.containsKey("oled_dim_start")) rtCfg.dimStartHour = doc["oled_dim_start"];
  if (doc.containsKey("oled_dim_end")) rtCfg.dimEndHour = doc["oled_dim_end"];
  if (doc.containsKey("sync_interval_ms")) rtCfg.syncIntervalMs = doc["sync_interval_ms"];
  if (doc.containsKey("ota_check_interval_ms")) rtCfg.otaCheckIntervalMs = doc["ota_check_interval_ms"];
}
...
void checkOtaUpdate() {
  ...
  if (strcmp(ver, FIRMWARE_VERSION) <= 0) return;
...
void performOtaUpdate() {
  ...
  if (strlen(otaState.md5) > 0) Update.setMD5(otaState.md5);
  uint8_t buff[1024];
  int written = 0;
...
SyncFileResult syncQueueFile(const char *fn) {
  if (!sdCardAvailable || !isWifiConnected()) return SYNC_FILE_NO_WIFI;
...
void chunkedSync() {
  ...
  while (syncState.currentFile < MAX_QUEUE_FILES && syncState.filesProcessed < MAX_SYNC_FILES_PER_CYCLE) {
    ...
    if (!exists) {
      ...
      emptyStreak++;
      if (emptyStreak >= 20) {
        // 20 file kosong berturut-turut — tidak ada data lagi
...
bool connectToWifi(int ssidIdx) {
  if (strlen(wifiCreds[ssidIdx].ssid) == 0) return false;
  WiFi.mode(WIFI_STA);
...
bool pingAPI() {
  ...
  Serial.println("PING: calling http.begin...");
...
bool kirimPresensi(const char *rfid, char *msg) {
  ...
  time_t now = time(nullptr);
    ...
    // Cek NVS dulu — tidak terpengaruh sync/delete file
  ...
  if (sdCardAvailable) {
    ...
    switch (r) {
      case SAVE_OK:
        ...
        return true;
      case SAVE_DUPLICATE: strcpy(msg, "CUKUP SEKALI!"); return false;
      case SAVE_QUEUE_FULL: strcpy(msg, "QUEUE PENUH!"); return false;
      default: strcpy(msg, "SD CARD ERROR"); return false;
    }
  }
  if (isWifiConnected()) {
...
void checkFactoryReset() {
    ...
    // Debounce kedua — pastikan bukan noise I2C
    vTaskDelay(pdMS_TO_TICKS(100));
    if (digitalRead(PIN_BOOT) != LOW) return;
...
static String provHtmlPage() {
  ...
  html += F("<br><b>Nama Device</b><br><input name='devname' placeholder='Nama Device (contoh: LOBBY_A)'>");
  html += F("<br><b>TLS SHA256 Fingerprint (opsional)</b><br><input name='tls_fp' placeholder='AA:BB:CC:...'>");
...
void taskRfid(void *param) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(xRfidQueue, &ev, pdMS_TO_TICKS(10)) == pdTRUE) {
      ...
      if (strcmp(rfidBuf, lastUID) == 0 && millis() - timers.lastScan < DEBOUNCE_TIME) continue;
    ...
    if (isWifiConnected()) {
      ...
      if (sdCardAvailable) {
        if (syncState.inProgress) chunkedSync();
          ...
          refreshPendingCache();
          timers.lastSync = now;
          if (cachedPendingRecords > 0) chunkedSync();
        }
      }
    }
    periodicTimeSync();
...
void setup() {
  ...
  const esp_task_wdt_config_t wdtCfg = {
    .timeout_ms = 180000u,  // 3 menit — cukup untuk OTA dan sync
    .idle_core_mask = 0,
    .trigger_panic = true
};
  ...
  loadCredentials();
  // Debug — hapus setelah confirmed OK
  ...
  if (strlen(deviceName) > 0) snprintf(deviceId, sizeof(deviceId), "%s", deviceName);
  ...
  if (!wifiOk) {
    ...
    if (isOnline && !isSignalCritical()) {
      ...
      if (sdCardAvailable && isWifiConnected()) {
        ...
        else showOLED(F("RFID DB"), "UP TO DATE");
        delay(600);
        esp_task_wdt_reset();
      }
    } else {
