...
4. **Manajemen Penyimpanan (Queue System):** Saat ada SD card dan RFID valid, data masuk ke antrean CSV (`queue_X.csv`) tanpa panggilan jaringan. Metadata antrean disimpan di `queue_meta.txt` untuk efisiensi akses. Saat antrean penuh (semua 2000 slot terpakai), tap ditolak dengan pesan `QUEUE PENUH!`.
...
9. **OTA Update Otomatis:** Perangkat memeriksa ketersediaan firmware terbaru ke server setiap 3 jam. Jika tersedia, firmware diunduh dan di-flash secara otomatis. Jika firmware baru crash sebelum `setup()` selesai, bootloader otomatis rollback ke firmware sebelumnya.
...
- **OTA Update dengan Rollback Otomatis:** Perangkat memeriksa firmware terbaru ke server setiap 3 jam. Pemeriksaan pertama dilakukan langsung saat boot (`lastOtaCheck = 0`). Jika tersedia, firmware diunduh dan di-flash tanpa intervensi manual. WDT dinonaktifkan selama proses download. Firmware baru ditandai valid via `esp_ota_mark_app_valid_cancel_rollback()` di awal `setup()` — jika firmware baru crash sebelum baris ini, bootloader ESP32 otomatis rollback ke firmware sebelumnya.
...
- **Queue Overwrite Protection:** Rotasi file antrean tidak menimpa file yang belum ter-sync. Jika semua slot penuh, tap ditolak dengan pesan `QUEUE PENUH!` yang informatif.
...
// Konfigurasi Duplikasi
// MAX_DUPLICATE_CHECK_LINES = MAX_RECORDS_PER_FILE + 1 (otomatis, tidak perlu diubah)
...
2. **Rotasi dengan Proteksi:** Saat berkas `queue_N` penuh, sistem membuat `queue_N+1`. Jika file target masih berisi record belum ter-sync, rotasi dibatalkan dan tap ditolak dengan pesan `QUEUE PENUH!`.
...
4. **Sliding Window Duplicate Check:** Memeriksa 3 berkas antrean terakhir untuk akurasi di boundary file. Batas baris per file dihitung dinamis sebagai `MAX_RECORDS_PER_FILE + 1`. `esp_task_wdt_reset()` ditempatkan di setiap iterasi file dan baris untuk mencegah WDT timeout.
...
6. **Return Code Terstruktur:** `saveToQueue()` mengembalikan enum `SaveResult` (`SAVE_OK`, `SAVE_DUPLICATE`, `SAVE_QUEUE_FULL`, `SAVE_SD_ERROR`) sehingga pesan error ke pengguna selalu akurat.
...
5. **Pembersihan Aman:** `nvsSetCount(0)` dieksekusi sebelum loop delete record — mencegah inkonsistensi count jika terjadi WDT reset di tengah proses penghapusan.
6. **Duplicate Check Aman:** Pengecekan duplikat menggunakan guard `unixTime >= rec.unixTime` sebelum unsigned subtraction untuk mencegah wraparound jika waktu sistem melompat mundur.

## Mekanisme Timekeeping (Hybrid)

Sistem menggunakan tiga lapis mekanisme untuk menjaga akurasi waktu. **Urutan eksekusi lapis 2 → lapis 3 adalah kritis dan tidak boleh diubah.**
...
2. **Kompensasi Deep Sleep (RTC RAM):** Sebelum masuk deep sleep, durasi tidur disimpan ke `sleepDurationSeconds` di RTC RAM. Saat bangun, `lastValidTime` dikompensasi dengan durasi tersebut sebelum digunakan, mengatasi `millis()` yang reset ke 0 setelah deep sleep. Nilai terkompensasi langsung disimpan ke NVS.

3. **Recovery Reset Paksa (NVS Flash):** Jika RTC RAM hilang akibat power putus atau reset paksa, `lastValidTime` dipulihkan dari NVS flash. Lapis ini hanya aktif jika lapis 2 tidak berlaku (bukan bangun dari deep sleep). Waktu ini mungkin tidak presisi karena tidak tahu durasi power mati, namun mencegah kondisi `WAKTU INVALID` dan langsung dikoreksi begitu NTP sync berhasil.
...
4. **Validasi Digit Konsisten:** Baik saat download (`downloadRfidDb()`) maupun saat load dari file (`loadRfidCacheFromFile()`), setiap baris divalidasi bahwa semua 10 karakter adalah digit. Baris non-digit dibuang di kedua alur.
5. **Validasi saat Tap:** Lookup dilakukan di RAM via `isRfidInCache()` — tanpa akses SD card dan tanpa HTTP call. Jika RFID tidak ditemukan di cache RAM, tap langsung ditolak dengan pesan `HUBUNGI ADMIN`.
6. **Fallback DB tidak ada:** Jika `rfid_db.txt` belum ada atau gagal dimuat ke RAM, `isRfidInCache()` mengembalikan `true` — semua tap diizinkan masuk ke queue, validasi diserahkan ke server saat sync.
7. **Siklus hidup cache RAM:** Cache dimuat saat boot setelah SD init, di-reload setelah download DB baru, di-reload setelah SD card kembali terbaca, dan di-free saat SD card terlepas.
8. **Pembaruan Berkala:** Setiap 3 jam, `checkAndUpdateRfidDb()` dipanggil dari loop. Cek versi dilakukan terlebih dahulu; download hanya dilakukan jika versi server lebih baru dari versi lokal.
...
6. **Rollback Otomatis:** `esp_ota_mark_app_valid_cancel_rollback()` dipanggil di awal `setup()`. Jika firmware baru crash sebelum mencapai baris ini, ESP32 bootloader otomatis rollback ke firmware sebelumnya pada reboot berikutnya.
7. **Hasil:** Sukses → restart otomatis dengan firmware baru, lalu ditandai valid saat boot. Gagal download/flash → `otaState.updateAvailable` di-reset, perangkat lanjut beroperasi normal.
...
```
Startup Animation
    └─ Recovery Waktu (URUTAN KRITIS: Lapis 2 → Lapis 3)
...
RFID terbaca
    └─ Ada SD card?
        ...
        │       ├─ Ditemukan → saveToQueue()
        │       │   ├─ SAVE_OK        → "DATA TERSIMPAN" / "QUEUE HAMPIR PENUH!"
        │       │   ├─ SAVE_DUPLICATE → "CUKUP SEKALI!"
        │       │   ├─ SAVE_QUEUE_FULL→ "QUEUE PENUH!"
        │       │   └─ SAVE_SD_ERROR  → "SD CARD ERROR"
...
Boot
    └─ initSDCard() selesai → loadRfidCacheFromFile()
        ├─ rfid_db.txt ada → validasi digit tiap baris → muat ke RAM heap → tampil "X RFID"
...
Boot / Loop (setiap 3 jam)
    └─ checkAndUpdateRfidDb()
        ...
        │       ├─ Streaming write ke /rfid_db.tmp (validasi digit tiap baris)
    ...
    └─ checkOtaUpdate()
        ...
        └─ Ada update → simpan ke otaState → tampil di OLED
            └─ Loop berikutnya (jika !rfidFeedback.active)
                └─ performOtaUpdate()
                    ├─ Sukses → ESP.restart()
                    │   └─ Boot dengan firmware baru
                    │       └─ setup(): esp_ota_mark_app_valid_cancel_rollback()
                    │           ├─ Tercapai → firmware ditandai valid, operasi normal
                    │           └─ Crash sebelum tercapai → bootloader rollback otomatis
...
setup() dipanggil
    │
    ├─ [LAPIS 2] sleepDurationSeconds > 0? (bangun dari deep sleep)
    │   └─ Ya
    ...
    │
    └─ [LAPIS 3] timeWasSynced == false || lastValidTime == 0? (reset paksa)
        └─ Ya (hanya jika Lapis 2 tidak aktif)
...
Library bawaan ESP32 core (tidak perlu install terpisah): `WiFi`, `WiFiClientSecure`, `HTTPClient`, `HTTPUpdate`, `Wire`, `SPI`, `time`, `esp_task_wdt`, `esp_ota_ops`, `Preferences`.

> **Catatan:** Gunakan ESP32 Arduino core v3.x. API `esp_task_wdt_init` pada core v3.x menggunakan struct `esp_task_wdt_config_t`. `esp_ota_ops.h` tersedia di ESP-IDF yang dibundel bersama ESP32 Arduino core v3.x.
...
| Duplicate Check Lines | MAX_RECORDS_PER_FILE + 1 (26) |
| Sync Interval         | 300 detik       |
| Duplicate Interval    | 1.800 detik     |
| Return Code           | Enum `SaveResult` (OK / DUPLICATE / QUEUE_FULL / SD_ERROR) |
...
| Delete order     | `nvsSetCount(0)` sebelum loop delete |
...
| Validasi digit         | Diterapkan di `downloadRfidDb()` dan `loadRfidCacheFromFile()` (kedua pass) |
| RAM cache              | Array pointer `char**` di heap, dimuat saat boot |
| RAM usage (2000 RFID)  | ~30KB heap                                |
...
| Rollback             | Otomatis via `esp_ota_mark_app_valid_cancel_rollback()` |
...
| WDT Reset Coverage     | Setiap iterasi loop operasi panjang |
| OTA Rollback           | Otomatis (bootloader ESP32) |
...
| Queue Full Message     | `QUEUE PENUH!` (informatif) |
| Time Persistence       | RTC RAM + NVS flash (dual layer) |
| NVS Delete Safety      | Count di-reset sebelum loop delete |
| Duplicate Check Safety | Guard unsigned subtraction wraparound |
...
| Tap Latency (Ada SD, RFID tidak ada di cache RAM)  | < 50ms           |
...
**Masalah: `esp_ota_mark_app_valid_cancel_rollback` not declared**
Pastikan `#include <esp_ota_ops.h>` ada di bagian atas firmware dan menggunakan ESP32 Arduino core v3.x dengan ESP-IDF yang dibundel.
...
**Masalah: Tap ditolak dengan pesan QUEUE PENUH!**
Semua 2000 slot file antrean SD card terpakai dan belum ter-sync ke server. Pastikan koneksi WiFi dan server dalam kondisi baik. Antrean akan otomatis dikosongkan setelah sync berhasil.
...
Firmware baru crash sebelum `esp_ota_mark_app_valid_cancel_rollback()` di `setup()` tercapai — bootloader otomatis rollback ke firmware sebelumnya. Periksa firmware baru untuk bug yang menyebabkan crash saat boot. Jika rollback tidak terjadi, pastikan partisi OTA dikonfigurasi dengan benar di partition table.
...
### v2.2.11 (Maret 2026)
- **[Fix]** Unsigned subtraction wraparound di `nvsIsDuplicate()`: tambah guard `unixTime >= rec.unixTime` sebelum subtraction untuk mencegah false negative saat waktu sistem melompat mundur setelah recovery NVS
- **[Fix]** OTA rollback otomatis: tambah `esp_ota_mark_app_valid_cancel_rollback()` di awal `setup()` dan `#include <esp_ota_ops.h>`; firmware baru yang crash sebelum boot selesai akan di-rollback otomatis oleh bootloader ESP32
- **[Fix]** Validasi digit konsisten di `loadRfidCacheFromFile()`: tambah loop `isdigit()` pada pass 1 dan pass 2, identik dengan logika di `downloadRfidDb()`; mencegah baris non-digit masuk ke RAM cache
- **[Fix]** Eliminasi duplikasi logika `isTimeValid()`: direfactor menjadi satu baris delegate ke `getTimeWithFallback()` — satu sumber kebenaran untuk logika fallback waktu
- **[Fix]** Race condition NVS delete: `nvsSetCount(0)` dipindah ke sebelum loop `nvsDeleteRecord()` untuk mencegah inkonsistensi count jika WDT reset terjadi di tengah iterasi penghapusan
- **[Fix]** Pesan error queue penuh yang menyesatkan: `saveToQueue()` kini mengembalikan enum `SaveResult` (`SAVE_OK`, `SAVE_DUPLICATE`, `SAVE_QUEUE_FULL`, `SAVE_SD_ERROR`); `kirimPresensi()` menggunakan `switch` untuk menampilkan pesan yang akurat per kondisi
- **[Fix]** `MAX_DUPLICATE_CHECK_LINES` diganti dari nilai hardcode `100` menjadi `MAX_RECORDS_PER_FILE + 1` (26) agar relevan dan tidak menyesatkan
- **[Fix]** Komentar eksplisit dependency urutan kritis lapis timekeeping di `setup()`: lapis 2 (kompensasi sleep) harus dieksekusi sebelum lapis 3 (recovery NVS) dengan penjelasan alasan
- Update versi string ke `2.2.11`

### v2.2.10 (Maret 2026)
- Perbaikan bug waktu setelah bangun dari deep sleep: durasi sleep kini disimpan ke `sleepDurationSeconds` (RTC RAM) sebelum `esp_deep_sleep_start()` dan dikompensasikan ke `lastValidTime` saat `setup()`
- Tambah persistensi waktu ke NVS flash (`last_time`): `lastValidTime` disimpan ke NVS setiap kali NTP sync berhasil
- Tambah `nvsSaveLastTime()` dan `nvsLoadLastTime()` untuk baca/tulis NVS key `last_time`
- Tambah `Dual Build Mode`: preprocessor `#define DEV_MODE` untuk switching HTTP/HTTPS
- Update versi string ke `2.2.10`

### v2.2.9 (Maret 2026)
- Tambah fitur RFID Local Database untuk validasi offline tanpa panggilan jaringan saat tap
- Tambah endpoint `/api/presensi/rfid-list` dan `/api/presensi/rfid-list/version`
- Tambah fungsi `downloadRfidDb()`, `isRfidInCache()`, `checkAndUpdateRfidDb()`
- Versi database disimpan di NVS key `rfid_db_ver`
- Update versi string ke `2.2.9`

### v2.2.8 (Maret 2026)
- Tambah fitur OTA Update otomatis via endpoint `/api/presensi/firmware/check`
- Tambah `OtaState` struct dan `lastOtaCheck` timer
- WDT dinonaktifkan selama proses OTA download
- Perbaikan WDT: tambah `esp_task_wdt_reset()` di iterasi loop panjang
- Update versi string ke `2.2.8`
