...
| `SaveResult` | Enum return type dari `saveToQueue()`: `SAVE_OK`, `SAVE_DUPLICATE`, `SAVE_QUEUE_FULL`, `SAVE_SD_ERROR` — membedakan jenis kegagalan penyimpanan queue secara eksplisit |
...
**FR-08** — Saat boot, `esp_ota_mark_app_valid_cancel_rollback()` dipanggil sesegera mungkin di awal `setup()` sebelum inisialisasi lainnya, menandai firmware aktif sebagai valid dan membatalkan mekanisme rollback otomatis bootloader. Jika firmware baru crash sebelum baris ini tercapai, bootloader akan otomatis rollback ke firmware sebelumnya.

**FR-09** — Setelah OTA mark valid, sistem menjalankan dua mekanisme recovery waktu secara berurutan sebelum inisialisasi SPI. Urutan ini kritis dan tidak boleh diubah: lapis 2 (deep sleep) harus dieksekusi sebelum lapis 3 (reset paksa) agar nilai `lastValidTime` yang dikompensasi tersimpan ke NVS sebelum lapis 3 berpotensi membaca NVS dengan nilai lama.
...
**FR-16** — Presensi ditolak dengan `"WAKTU INVALID"` jika tidak ada sumber waktu valid. `isTimeValid()` tidak menduplikasi logika — diimplementasikan sebagai satu baris yang mendelegasikan ke `getTimeWithFallback()` sebagai satu-satunya sumber kebenaran validitas waktu.
...
**FR-21** — Duplicate check dilakukan pada 3 file queue terakhir dengan window 30 menit dan batas `MAX_RECORDS_PER_FILE + 1` baris per file (mencakup tepat satu header CSV ditambah seluruh record maksimal).

**FR-22** — `saveToQueue()` mengembalikan enum `SaveResult` (`SAVE_OK`, `SAVE_DUPLICATE`, `SAVE_QUEUE_FULL`, `SAVE_SD_ERROR`). `kirimPresensi()` membaca enum ini untuk menampilkan pesan yang akurat: `"QUEUE PENUH!"` untuk `SAVE_QUEUE_FULL` dan `"SD CARD ERROR"` untuk `SAVE_SD_ERROR`.

**FR-23** — Rotasi file dilakukan saat file aktif mencapai 25 record. Rotasi dibatalkan dan mengembalikan `SAVE_QUEUE_FULL` jika file target masih berisi data belum ter-sync.
...
**FR-32** — `loadRfidCacheFromFile()` mengalokasikan array pointer `char**` di heap, kemudian mengalokasikan 11 byte per RFID. Validasi digit (`isdigit()`) diterapkan pada pass 1 (penghitungan) maupun pass 2 (pengisian data), konsisten dengan logika di `downloadRfidDb()`. Total penggunaan heap untuk 2000 RFID sekitar 22KB.
...
**FR-34** — NVS buffer disync ke server SEBELUM SD queue (prioritas NVS lebih tinggi). Saat sync berhasil, `nvsSetCount(0)` dipanggil sebelum loop delete record individual untuk mencegah kondisi count tidak konsisten jika WDT reset terjadi di tengah iterasi delete.
...
**FR-40** — OTA dieksekusi otomatis tanpa konfirmasi, hanya jika tidak ada tap RFID aktif (`!rfidFeedback.active`). Setelah firmware baru berhasil di-flash dan ESP restart, `esp_ota_mark_app_valid_cancel_rollback()` di awal `setup()` akan menandai firmware baru sebagai valid.
...
- `nvsIsDuplicate()` menggunakan guard `unixTime >= rec.unixTime` sebelum subtraction untuk mencegah unsigned integer wraparound jika waktu sistem melompat mundur setelah recovery NVS
...
| `#define FIRMWARE_VERSION` | `"2.2.11"` | Versi firmware aktif, dikirim saat OTA check |
| `MAX_DUPLICATE_CHECK_LINES` | `MAX_RECORDS_PER_FILE + 1` (= 26) | Batas baris cek duplikat per file — satu header + maks record |
...
| Queue penuh (semua 2000 slot terpakai) | `INFO` | `QUEUE PENUH!` | 3× beep |
...
| esp_ota_ops.h | ESP32 core | OTA rollback control (`esp_ota_mark_app_valid_cancel_rollback`) |
