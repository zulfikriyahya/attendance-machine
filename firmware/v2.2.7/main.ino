/*
 * ======================================================================================
 * SISTEM PRESENSI PINTAR (RFID) - QUEUE SYSTEM v2.2.7
 * ======================================================================================
...
// NVS BUFFER CONFIG (fallback: tanpa SD card)
...
// ========================================
// NVS BUFFER (fallback tanpa SD card)
// ========================================
int nvsGetCount()
{
...
// ========================================
// RFID VALIDATION (online, hanya untuk kondisi tanpa SD)
// ========================================
RfidValidResult validateRfidOnline(const char *rfid)
{
    if (WiFi.status() != WL_CONNECTED)
...
bool kirimPresensi(const char *rfidUID, char *message)
{
    ...
    // Ada SD card — langsung queue, tanpa validasi jaringan
    if (sdCardAvailable)
    {
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        if (valid == RFID_VALID)
        {
            ...
            // Server down / error — simpan ke NVS buffer
            ...
            if (nvsSaveToBuffer(rfidUID, timestamp, currentUnixTime))
            {
                ...
                snprintf(buf, sizeof(buf), "BUFFER %d/%d", nvsCount, NVS_MAX_RECORDS);
                strcpy(message, buf);
                return true;
            }
        ...
        // RFID_UNREACHABLE — server tidak bisa dihubungi sama sekali
        if (nvsIsDuplicate(rfidUID, currentUnixTime))
        {
            strcpy(message, "CUKUP SEKALI!");
            return false;
...
void showStartupAnimation()
{
    ...
    static const char version[] = "v2.2.7";
...
void updateCurrentDisplayState()
{
    ...
    refreshPendingCache();
    int nvsPending = nvsGetCount();
    currentDisplay.pendingRecords = cachedPendingRecords + nvsPending;
    if (WiFi.status() == WL_CONNECTED)
    {
        long rssi = WiFi.RSSI();
...
void loop()
{
    ...
    if (now - timers.lastPeriodicCheck >= PERIODIC_CHECK_INTERVAL)
    {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            // Sync NVS buffer jika ada (prioritas sebelum SD sync)
