...
[checkOtaUpdate() — guard 3 jam, langsung saat boot]
  ...
  │    Body: { version: "2.2.9", device_id: "ESP32_XXYY" }
...
**FR-04** — WiFi dikoneksikan dengan maksimal 20 retry × 300ms. Jika berhasil, ping API dilakukan maksimal 3 retry. Jika API OK, urutan: sync NTP → sync NVS → sync SD queue → cek & update RFID DB.
...
**FR-08** — RFID dipindai setiap iterasi loop dengan prioritas tertinggi. Jika ada kartu, `handleRFIDScan()` dipanggil dan loop di-return.

**FR-09** — UID dikonversi ke 10 digit desimal via konversi little-endian 4 byte.

**FR-10** — Debounce 150ms untuk UID yang sama diterapkan sebelum proses apapun.
...
**FR-17** — Rotasi file dilakukan saat file aktif mencapai 25 record. Rotasi dibatalkan jika file target masih berisi data belum ter-sync.

**FR-18** — Jika SD tidak ada dan WiFi tersedia, data dikirim langsung ke `/api/presensi`. Jika gagal, data di-fallback ke NVS buffer.

**FR-19** — Jika SD tidak ada dan WiFi tidak ada, data disimpan ke NVS buffer (max 20 record).

**FR-20** — Setiap tap kartu SELALU menghasilkan feedback OLED + buzzer tanpa terkecuali. Feedback ditampilkan minimal 1800ms.

### 4.3 RFID Local Database

**FR-21** — Database RFID lokal disimpan di `/rfid_db.txt` dengan format satu RFID (10 digit) per baris.

**FR-22** — Versi database disimpan di NVS key `rfid_db_ver` sebagai `unsigned long` (timestamp Unix dari server).

**FR-23** — Sebelum download, perangkat membandingkan versi lokal dengan server via GET `/api/presensi/rfid-list/version`. Download hanya dilakukan jika `serverVer > localVer`.

**FR-24** — Download dilakukan dengan streaming chunk 256 byte langsung ke file sementara `/rfid_db.tmp` tanpa memuat seluruh response ke heap. Setelah selesai, file di-rename ke `/rfid_db.txt` untuk menjaga atomicity.
...
**FR-27** — NVS buffer disync ke server SEBELUM SD queue (prioritas NVS lebih tinggi).

**FR-28** — Chunked sync SD berjalan non-blocking: max 5 file per siklus, max 15 detik per siklus.

**FR-29** — Record lebih dari 30 hari dibuang saat sync, tidak dikirim ke server.

**FR-30** — Record ditolak server dicatat ke `failed_log.csv` di SD card.

**FR-31** — Sync berjalan di latar belakang tanpa feedback visual atau audio ke pengguna.

### 4.5 OTA Update

**FR-32** — OTA check dilakukan setiap 3 jam dan langsung saat boot pertama (`lastOtaCheck = 0`).

**FR-33** — OTA dieksekusi otomatis tanpa konfirmasi, hanya jika tidak ada tap RFID aktif (`!rfidFeedback.active`).

**FR-34** — WDT dinonaktifkan selama proses download dan flashing OTA.

**FR-35** — Jika OTA gagal, WDT diinisialisasi ulang dan sistem melanjutkan operasi normal.

### 4.6 Manajemen Daya

**FR-36** — OLED dimatikan jam 08:00–14:00 untuk menghemat daya baterai UPS.

**FR-37** — Saat OLED mati dan ada tap kartu, OLED menyala sementara untuk feedback lalu kembali ke state jadwal.

**FR-38** — Deep sleep aktif jam 18:00–05:00. Durasi dihitung tepat ke jam 05:00, min 60 detik, max 12 jam.

**FR-39** — Jika ada sync berjalan saat jam sleep, sync diselesaikan dulu sebelum masuk sleep.

**FR-40** — WDT dinonaktifkan sebelum `esp_deep_sleep_start()`. Safety net reinit WDT ditempatkan setelah pemanggilan tersebut untuk kondisi sleep gagal.

### 4.7 Kesehatan Sistem

**FR-41** — SD card di-health check setiap 30 detik via `fatType()`.
...
| RFID tidak terdaftar (lokal + server) | `INFO` | `HUBUNGI ADMIN` | 3× beep |
