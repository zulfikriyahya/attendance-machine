/*
 * ======================================================================================
 * SISTEM PRESENSI PINTAR (RFID) - QUEUE SYSTEM v2.2.10 (With OTA + RFID Local DB)
 * ======================================================================================
...
#define OTA_CHECK_INTERVAL 21600000UL
...
RTC_DATA_ATTR uint64_t sleepDurationSeconds = 0; // kompensasi millis() reset setelah deep sleep
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        while (millis() - start < 2500)
        {
            ...
            if (getLocalTime(&timeInfo) && timeInfo.tm_year >= 120)
            {
                ...
                nvsSaveLastTime(lastValidTime); // persist ke NVS untuk ketahanan reset paksa
                timeWasSynced = true;
                if (!bootTimeSet)
                {
                    bootTime = millis();
...
{
    prefs.begin(NVS_NAMESPACE, true);
    unsigned long ver = prefs.getULong(NVS_KEY_RFID_VER, 0);
    prefs.end();
    return ver;
}
...
bool downloadRfidDb()
{
    ...
    const char *tmpFile = "/rfid_db.tmp";
    if (sd.exists(tmpFile))
        sd.remove(tmpFile);

    FsFile dbFile;
    if (!dbFile.open(tmpFile, O_WRONLY | O_CREAT | O_TRUNC))
    {
        deselectSD();
        releaseSD();
        http.end();
    ...
    while (http.connected() && (totalSize < 0 || written < totalSize))
    {
        ...
        int toRead = min(available, (int)sizeof(chunk));
        ...
        for (int i = 0; i < read; i++)
        {
            ...
            {
                if (lineBufPos < (int)sizeof(lineBuf) - 1)
    ...
    sd.rename(tmpFile, RFID_DB_FILE);
...
void checkAndUpdateRfidDb()
{
    ...
    if (serverVer == 0 || serverVer <= localVer)
        return;
...
void showStartupAnimation()
{
    ...
    static const char version[] = "v2.2.10";
...
void setup()
{
    ...
    // Kompensasi waktu setelah bangun dari deep sleep
    // millis() reset ke 0 setelah deep sleep, sehingga bootTime lama tidak valid
    ...
    // Pulihkan waktu dari NVS jika RTC RAM hilang akibat reset paksa / power putus
...
void loop()
{
    ...
    if (getTimeWithFallback(&timeInfo))
    {
        ...
        if (h >= SLEEP_START_HOUR || h < SLEEP_END_HOUR)
        {
            ...
            // Simpan durasi sleep ke RTC RAM untuk kompensasi waktu saat bangun
