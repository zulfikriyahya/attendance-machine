...
int _countRecordsInternal(const String& filename) {
  if (!file.open(filename.c_str(), O_RDONLY)) return 0;
...
bool isDuplicateInAllQueues(const char* rfid, unsigned long currentUnixTime) {
  ...
  for (int i = startFile; i <= currentQueueFile; i++) {
    String filename = getQueueFileName(i);
    if (!sd.exists(filename.c_str())) continue;
    if (!file.open(filename.c_str(), O_RDONLY)) continue;
...
bool saveToQueue(const char* rfid, const char* timestamp, unsigned long unixTime) {
  if (!sdCardAvailable) return false;
  if (isDuplicateInAllQueues(rfid, unixTime)) return false;
...
bool syncAllQueues() {
  ...
  for (int i = 0; i < MAX_QUEUE_FILES; i++) {
    ...
    snprintf(messageBuffer, sizeof(messageBuffer), "File %d (%d)", i, fileRecords);
...
void periodicTimeSync() {
  if (millis() - lastTimeSyncAttempt < TIME_SYNC_INTERVAL) return;
...
void showStartupAnimation() {
  ...
  const char version[] PROGMEM = "v2.2.0 ULTIMATE";
...
void showStandbySignal() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    long rssi = WiFi.RSSI();
    int bars = (rssi > -67) ? 4 : (rssi > -70) ? 3 : (rssi > -80) ? 2 : (rssi > -90) ? 1 : 0;
    for (int i = 0; i < 4; i++) {
      int h = 2 + i * 2;
      int x = SCREEN_WIDTH - 18 + i * 5;
      if (i < bars) display.fillRect(x, 10 - h, 3, h, WHITE);
      else display.drawRect(x, 10 - h, 3, h, WHITE);
    }
  }
...
void loop() {
  ...
  if (getTimeWithFallback(&timeInfo)) {
    ...
    if (h >= SLEEP_START_HOUR || h < SLEEP_END_HOUR) {
      ...
      int sleepSec = (h >= SLEEP_START_HOUR) ? 
        ((24 - h + SLEEP_END_HOUR) * 3600 - timeInfo.tm_min * 60) :
        ((SLEEP_END_HOUR - h) * 3600 - timeInfo.tm_min * 60);
