/*
 * ======================================================================================
 * SISTEM PRESENSI PINTAR (RFID) - QUEUE SYSTEM v2.2.4 ULTIMATE
 * ======================================================================================
 ...
 *
 * CHANGELOG v2.2.4:
 * - OLED Auto Dim: Mati pukul 08:00, Nyala pukul 14:00
 * ======================================================================================
 */
...
bool oledIsOn = true; // Status OLED
...
void turnOnOLED()
{
  if (!oledIsOn)
  {
    ...
    oledIsOn = true;
    currentDisplay.needsUpdate = true; // Force update tampilan
  }
}
...
void checkOLEDSchedule()
{
  ...
  {
    return; // Jika waktu tidak tersedia, skip
  }

  int currentHour = timeInfo.tm_hour;

  // Cek apakah waktu saat ini dalam rentang DIM (08:00 - 13:59)
...
// ========================================
// NETWORK FUNCTIONS (OPTIMIZED FOR MAX SIGNAL)
// ========================================
bool connectToWiFi()
{
  WiFi.mode(WIFI_STA);
  ...
  // ===== OPTIMASI KEKUATAN SINYAL =====
  // Set TX Power ke maksimum untuk jangkauan
...
void processReconnect()
{
  ...
  case RECONNECT_INIT_SSID1:
    WiFi.disconnect(true);
    delay(100);
    // Set power maksimal sebelum reconnect
    WiFi.setTxPower(WIFI_POWER_19_5dBm);
    WiFi.setSleep(WIFI_PS_MAX_MODEM);
    {
      char ssid[16], password[16];
...
void showStartupAnimation()
{
  ...
  const char version[] PROGMEM = "v2.2.4";
