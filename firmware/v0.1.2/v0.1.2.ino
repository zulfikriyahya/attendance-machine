...
void setup() {
  ...
  struct tm timeinfo;
  if (!getLocalTime(&timeinfo)) { fatalError("GAGAL AMBIL WAKTU"); }
  kirimDataOffline();
}
void loop() {
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    if (sukses) {
      ...
      simpanOfflineLog(rfidStr, String(buffer));
    }
    delay(150);
    rfid.PICC_HaltA();
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    if (jam >= 18 || jam < 5) {
      ...
      int detikSisa;
      if (jam >= 18) {
        detikSisa = ((24 - jam + 5) * 3600) - (timeinfo.tm_min * 60 + timeinfo.tm_sec);
      } else {
        detikSisa = ((5 - jam) * 3600) - (timeinfo.tm_min * 60 + timeinfo.tm_sec);
      }
...
void simpanOfflineLog(String rfid, String waktu) {
  if (!sdSiap) {
    Serial.println("SD BELUM SIAP");
    return;
  }
  ...
  if (file) {
    ...
    file.close();
    if (err) {
      Serial.println("Gagal parsing offline.json, membuat baru");
      doc.clear();
    }
  }
  ...
  if (file) {
    ...
    file.close();
    Serial.println("Presensi disimpan offline: " + rfid);
  } else {
    Serial.println("Gagal tulis offline.json");
  }
}
void kirimDataOffline() {
  ...
  file.close();
  if (err) {
    Serial.println("Gagal parsing offline.json");
    return;
  }
  JsonArray data = doc.as<JsonArray>();
  JsonArray dataGagal;
  for (JsonObject entry : data) {
    String rfid = entry["uid"];
    String waktu = entry["waktu"];
    String pesan, nama, status, wkt;
    if (!kirimPresensi(rfid, pesan, nama, status, wkt)) {
      JsonObject gagal = dataGagal.createNestedObject();
      gagal["uid"] = rfid;
      gagal["waktu"] = waktu;
    } else {
      Serial.println("Terkirim Offline: " + rfid);
    }
  }
  SD.remove("/offline.json");
  if (dataGagal.size() > 0) {
    File f = SD.open("/offline.json", FILE_WRITE);
    if (f) {
      serializeJsonPretty(dataGagal, f);
      f.close();
    }
  }
}
